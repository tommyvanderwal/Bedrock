"""Supportability checks (Stage 9 PR #2).
...
Dependency-injection pattern: ``load_cluster``, ``get_nodes``,
``ssh_cmd_rc`` come in via ``register_routes`` to avoid a circular
import on the still-monolithic ``mgmt/app.py``.
"""
...
def register_routes(
    app: FastAPI,
    *,
    load_cluster: Callable,
    get_nodes: Callable,
    ssh_cmd_rc: Callable,
) -> None:
    """Attach ``/api/support/checks`` to ``app``."""
