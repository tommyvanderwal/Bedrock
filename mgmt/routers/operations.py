...
This is the single surface the ``bedrock`` CLI talks to. Today's
``POST /api/vms`` etc. endpoints can gradually delegate here; the
legacy bodies stay for backward compat until they're empty shims.
...
# Make sure the bedrock_d package is importable when this module loads
# inside bedrock-d's running process (repo root = two levels up from
# mgmt/routers/; on deployed nodes bedrock-d's entrypoint already has
# /usr/local/lib/bedrock on sys.path).
...
@router.post("/api/operations/{op_id}/retry")
