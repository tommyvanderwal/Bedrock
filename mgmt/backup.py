...
# Per-VM replication via `kopia snapshot migrate` (the ONE replication path —
# sync-to is gone). A secondary copy re-ingests the VM's snapshots through the
# dest repo (decrypt source → re-chunk → re-encrypt to the dest key), so it
# works across different repo passwords + formats (a local public-default
# primary → an offsite real-password secondary). Generous timeout (first full
# copy walks the VM's whole history, deduped); incrementals top up only new.
MIGRATE_TIMEOUT_S = 14400      # 4h
MIGRATE_PARALLEL = 4           # kopia snapshot migrate --parallel (sources)
...
# ── per-VM replication via `kopia snapshot migrate` ───────────────────────


def _vm_migrate_sources(primary_id: str, vm_name: str) -> list:
    """The kopia source specs for ONE VM in the primary repo — every snapshot
    source matching ``…:vms:<vm>:*`` (each disk + the ``:metadata`` snapshot).
    Enumerated from the repo itself, so it captures exactly what was backed up
    with no disk re-resolution. Returns the distinct source paths to feed
    ``snapshot migrate --sources`` (kopia matches a VM's snapshots on this
    string and leaves the other VMs untouched — verified per-VM-selective)."""
    g = _kopia_global_flags(primary_id)
    env = _credentials_env(primary_id)
    cmd = f"{env} && kopia {g} snapshot list --all --json"
    r = subprocess.run(["bash", "-lc", cmd], capture_output=True, text=True,
                       timeout=120)
    if r.returncode != 0:
        raise RuntimeError(
            f"migrate: could not list snapshots in primary {primary_id!r}: "
            f"{(r.stderr or r.stdout or '').strip()[:300]}")
    try:
        snaps = json.loads(r.stdout or "[]")
    except json.JSONDecodeError:
        snaps = []
    needle = f":vms:{vm_name}:"
    sources = {(s.get("source") or {}).get("path", "") for s in snaps}
    return sorted(p for p in sources if needle in p)


def run_migrate_to_secondaries(primary_target_id: str,
                               secondary_target_ids: list,
                               *, vm_name: str) -> dict:
    """Replicate ONE VM's backup from the PRIMARY repo to each SECONDARY via
    ``kopia snapshot migrate`` — the single replication mechanism (sync-to is
    gone). Runs on the VM's home node.

    Per-VM by construction: only this VM's sources (disks + metadata) are
    copied. The DEST is the connected secondary repo with ITS OWN password +
    S3 creds; the SOURCE is read via ``--source-config`` with the primary's own
    persisted creds — so a local public-default primary replicates to an
    offsite real-password secondary, re-encrypting to the offsite key (migrate
    crosses passwords/formats; sync-to could not).

    Each secondary is migrated INDEPENDENTLY — one unreachable/failed offsite
    never aborts the others, and never the already-recorded primary backup.
    Idempotent: migrate skips snapshots the dest already has (manifest match by
    source+StartTime), so a retry only tops up what's new. Returns
    {ok:[ids], failed:[{target,reason}], results:[…], sources:[…]}."""
    cluster = _read_cluster()
    targets = cluster.get("backup_targets") or {}
    primary = targets.get(primary_target_id)
    if primary is None:
        # _read_cluster swallows rqlite errors → {}; the primary was just
        # written to, so an empty map means a transient read, not a real miss.
        if not targets:
            raise RuntimeError(
                f"migrate: could not read cluster state (rqlite transient/"
                f"no-leader?) — cannot resolve primary {primary_target_id!r}; "
                f"retry the operation")
        raise RuntimeError(
            f"migrate: primary target {primary_target_id!r} not in cluster "
            f"state — cannot replicate")
    # Resume-safe: the primary must be connected locally to read it as source.
    _ensure_local_connection(primary_target_id, primary)
    sources = _vm_migrate_sources(primary_target_id, vm_name)
    if not sources:
        log.info("migrate[%s]: no sources in primary %r — nothing to replicate",
                 vm_name, primary_target_id)
        return {"ok": [], "failed": [], "results": [], "sources": []}
    src_config = _kopia_config_file(primary_target_id)
    src_flags = " ".join(f"--sources={shlex.quote(s)}" for s in sources)
    ...
    for sec_id in (secondary_target_ids or []):
        ...
        if sec is None:
            ...
            log.error("migrate[%s]: secondary %r not in cluster state — "
                      "skipping (others continue)", vm_name, sec_id)
            continue
        try:
            # Connect the SECONDARY (the migrate dest) on this node with its own
            # config + creds + password (resolved from its endpoint via the view).
            _ensure_local_connection(sec_id, sec)
            g = _kopia_global_flags(sec_id)        # --config-file=<sec>.config
            env = _credentials_env(sec_id)         # dest KOPIA_PASSWORD + S3 env
            cmd = (f"{env} && kopia {g} snapshot migrate "
                   f"--source-config={shlex.quote(src_config)} "
                   f"{src_flags} --no-policies "
                   f"--parallel={int(MIGRATE_PARALLEL)}")
            r = subprocess.run(["bash", "-lc", cmd], capture_output=True,
                               text=True, timeout=MIGRATE_TIMEOUT_S)
            dur = time.monotonic() - t0
            if r.returncode != 0:
                msg = (r.stderr or r.stdout or "").strip()[-600:]
                results.append({"target_id": sec_id, "ok": False,
                                "duration_s": dur, "error": msg})
                log.error("migrate[%s] -> %s FAILED (rc=%d, %.1fs): %s",
                          vm_name, sec_id, r.returncode, dur, msg)
            else:
                results.append({"target_id": sec_id, "ok": True,
                                "duration_s": dur, "error": ""})
                log.info("migrate[%s] -> %s ok (%d source(s), %.1fs)",
                         vm_name, sec_id, len(sources), dur)
        except Exception as e:
            dur = time.monotonic() - t0
            results.append({"target_id": sec_id, "ok": False,
                            "duration_s": dur, "error": str(e)})
            log.error("migrate[%s] -> %s raised: %s", vm_name, sec_id, e)

    ok = [r["target_id"] for r in results if r["ok"]]
    failed = [{"target": r["target_id"], "reason": r["error"]}
              for r in results if not r["ok"]]
    return {"ok": ok, "failed": failed, "results": results, "sources": sources}
