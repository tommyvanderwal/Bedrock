...
- **INV-5** — the takeover UUID comparison (step 3, against M's slot) is **equality of the data GENERATION**, never an ordering — any genuine mismatch refuses. UUIDs carry no order. "Newer/older" exists only in the cold-boot self-slot check, and only as a local 7-day-history classification (`state.classify_arbiter_uuid`: superseded ⇒ refuse; current/unseen ⇒ allow), never as a numeric compare.
  - **The UUID must be read + compared the way DRBD itself does it (fixed 2026-06-05):**
    - **Source:** the LIVE current UUID = the **first whitespace token of the first line** of debugfs `data_gen_id`. Later tokens on that line are per-peer *bitmap* UUIDs and later lines are *history* UUIDs — reading those compares a different generation. `drbdadm dump-md` is used **only** when the resource is genuinely DETACHED (debugfs file absent — the N=1 scratch path); on an attached resource it can report a value that has since rolled into the bitmap, so a transient debugfs error there returns `""` (defer), never a stale guess. `netd._read_cluster_uuid` (which publishes the marker) and `cluster_arbiter._read_local_drbd_uuid` (which reads locally for step 3) share ONE reader, so the published marker and the local read can never be apples-vs-oranges.
    - **Role-bit mask:** DRBD overloads **bit 0 of the current UUID as the Primary-role flag** (set/cleared on role change *without* minting a new UUID), and masks `& ~((u64)1)` on every current-UUID comparison (`drbd_uuid_compare`/`receive_uuids` in LINBIT's source). We do the same (`_mask_drbd_role_bit`): the marker is published already-masked and step 3 re-masks defensively. Without it an in-sync Secondary (`UpToDate`, `oos:0`) — whose current UUID differs from the ex-Primary's published marker by *only* bit 0 — is wrongly refused as "divergence." Masking bit 0 does NOT weaken divergence detection: bit 0 is role, not generation. (Confirmed live: Primary `…1B` vs Secondaries `…1A`, identical masked.)
  - **Sibling path fixed too (2026-06-05):** the pet-VM DRBD-UUID gate (VM-02) now shares the one reader — `bedrock_d/vm/failover.py::read_local_drbd_uuid` delegates to `cluster_arbiter._read_local_drbd_uuid(resource)` (parameterized by resource), the pre-start check masks the recorded marker before comparing, and `vm/migrate.py` masks + token-0s the UUID it records over SSH. So both the arbiter takeover and VM failover read/compare DRBD generations identically.
