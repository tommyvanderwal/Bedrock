...
|---|---|---|---|---|---|
| R1+R2 | **Multi-witness / even-split witness exclusivity** | **RESOLVED by the witness-as-death-oracle design** (`docs/witness-death-oracle.md`): the witness is a death-detector, not a contestable vote. A clean even split never moves the master (a far node defers to a FRESH+HOSTING slot unless it holds a node-majority); the only place two nodes could contend a witness is an even *survivor*-split, which is *proven never pivotal* (`deficit ≥ 51`, dead master still in the denominator). So no settle-barrier / SCSI-PR register / majority-of-witnesses read is needed. | — | Implement the death-oracle (`master_alive = reachable ∨ slot FRESH∧HOSTING`), node-majority-bounded deference, the slot `HOSTING` bit, and witness-inclusive self-demote. | resolved by design |
| R3 | **Hung (not dead) master + no DRBD fence handler** | INV-1's 9-vs-10 margin is time-based on two *different* nodes; a wedged old master (GC/`virsh`/`drbdadm` hang) may not self-demote at 9 while the survivor `--force primary`s at 10 → transient dual-Primary on `cluster` until DRBD reconnects (`after-sb-2pri disconnect`). No `fencing resource-and-stonith` / `fence-peer` handler exists | `on-no-quorum suspend-io`; generation gate; strong-read commit gates (so no data commit); **`auto-promote no` (2026-06-05) — DRBD never self-promotes, so the *only* way two Primaries arise is bedrock-d `--force`-promoting the survivor while the old Primary is wedged; the divergence is bounded by suspend-io + `after-sb` and the loser resyncs on reconnect** | Add a DRBD `fence-peer` handler (or a positive interlock) so the survivor's promote is blocked until the old Primary is provably down/fenced. *Availability* half only — the data-divergence half is now backstopped harder by `auto-promote no` | Med (availability) |
| R4 | **RESOLVED (2026-06-05).** Boot: arbiter LUN not `drbdadm up` on a returning node | Was: `converge()` only ran `drbdadm primary`, never `up`, so a rebooted node hit "Unknown resource" and could never host the arbiter until a manual `drbdadm up cluster`. | — | `cluster_arbiter.ensure_arbiter_drbd_up()` brings the resource up as **Secondary** on every tier-holder (gated on the `cluster-drbd-ready` marker), called eagerly from `_start_local_services` **and** every `converge()` tick (self-heal). No quorum / no Primary needed — peers resync on UUID lineage. Promote stays election-gated; `_drbd_up()` also runs before `_drbd_promote()`. Validated live: hard-reboot a follower → auto-up → `Connected`/`UpToDate` Secondary, zero manual steps. | resolved |
| R5 | **RESOLVED (down→up) + split-brain made unreachable (2026-06-05).** Recovery from degraded DRBD | The common degraded case — resource *down* after a cold boot/crash — is now the same `ensure_arbiter_drbd_up()` path (up→connect→resync; DRBD's own protocol heals Inconsistent→UpToDate). The *scary* case — StandAlone from a 2-primary divergence — is made **impossible by construction** by the R3 keystone `auto-promote no` (DRBD never self-promotes; the election/witness are the sole promote authority, so two Primaries can't coexist). If a StandAlone is *ever* observed it is logged **loud** (`log.error`) and left untouched for an operator — never silently discarded. | — | Diskless / missing meta-LV rebuild + dashboard `dstate`/`cstate` surfacing remain (tracked under self-heal / observability), but the split-brain-recovery hazard is closed. | resolved (split-brain); residual: meta-LV rebuild + dashboard surfacing |
...
cases — yes, the layered defense (§6) holds. R1+R2 are **resolved by the
death-oracle design**; R4 (boot up) and the split-brain half of R5 are
**resolved** (2026-06-05). The keystone for the latter is **`auto-promote no`**:
DRBD never picks Primary itself, so the cluster election + witness are the *sole*
promote authority and two Primaries cannot coexist — a 2-primary split-brain is
impossible by construction, not merely backstopped. What remains: the
*availability* half of R3 (a positive fence/interlock so a survivor's promote
waits on the old Primary being provably down), R5's Diskless/meta-LV rebuild +
dashboard `dstate`/`cstate` surfacing, and R6's `witness add` count cap.
