...
- `_vmagent_unit(metrics_backends, snapshot) -> str` / `_vlagent_unit(logs_backends, snapshot) -> str` — render the agent unit text, one `-remoteWrite.url` per reachable backend (`:8428/api/v1/write` for metrics, `:9428/internal/insert` for logs). vmagent reads `SCRAPE_FILE` via `-promscrape.config`; vlagent listens on TCP 5140 syslog (no systemd ordering vs rsyslog — rsyslog is the TCP client). Both set `-remoteWrite.maxDiskUsagePerURL=8GB`.
