...
Downloads and starts:
  ...
  - FastAPI + Svelte dashboard (8443 HTTPS LAN + 127.0.0.1:8001 loopback)
  - SQLite inventory DB
...
def install_full(cluster_name: str, repo: str):
    ...
    Setting ``BEDROCK_INIT_SAGA=0`` selects the procedural body below
    instead — a fallback for diagnosing a saga step bug.
    """
    import os as _os
...
""")
    ...
    # 127.0.0.1:8001 is loopback-only (intra-node); HTTPS on 8443 is the
    ...
    # the sole obs_backend. Every subsequent mutation rides this
    # same rqlite store + bumps revision.
    try:
        import time as _t
        import json as _json
        ...
        # Bootstrap chicken-egg: rqlite_setup.render_env_file()
        # reads cluster.json for this node's loopback_ip (node_id is
        # its last octet) and the peer list. cluster.json is normally
        # regenerated by view_builder from rqlite — but rqlite isn't
        # up yet at init time. So write a minimal cluster.json with
        # just the master entry, render the env, start rqlited, seed
        # the schema + initial rows, then let the orchestrator's
        # rqlite_subscriber rewrite cluster.json from the canonical
        # store going forward.
