...
LOG_PATH=OUTDIR/f'sweep-4node-confirm-{STAMP}.log'
...
# Best performers from 200-run campaign.
TOP_VARIANTS=[
 {'id':'c01','hot':14,'writers':36,'payload':16,'kill':0.60,'rounds':2,'settle':10}, # v05 10/10
 {'id':'c02','hot':16,'writers':36,'payload':16,'kill':0.60,'rounds':2,'settle':10}, # v11 10/10
 {'id':'c03','hot':14,'writers':36,'payload':12,'kill':0.60,'rounds':2,'settle':10}, # v15 10/10
 {'id':'c04','hot':16,'writers':32,'payload':16,'kill':0.75,'rounds':2,'settle':10}, # v17 10/10
]
...
def run_one(global_iter:int, variant:dict, rep:int):
    victim_idx=global_iter % len(ENDPOINTS)
    bucket_name=f"leak-confirm-{variant['id']}-{rep}-{global_iter}"
    env=os.environ.copy()
    env.update({
...
def main():
    total=len(TOP_VARIANTS)*REPEATS_PER_VARIANT
    fields=['iter','variant_id','repeat','victim_idx','hot_keys','writers_per_key','payload_mib','kill_delay_s','read_rounds','settle_s','duration_s','exit_code','baseline_fail','hot_fail','cold_fail','reproduced_strict','reproduced_hot_any','raw_tail']
    strict=anyh=0
    gi=0
    with CSV_PATH.open('w',newline='') as fc, LOG_PATH.open('w') as fl:
        w=csv.DictWriter(fc,fieldnames=fields); w.writeheader(); fc.flush()
        print(f"START total={total} csv={CSV_PATH} log={LOG_PATH}", flush=True)
        fl.write(f"START total={total} csv={CSV_PATH} log={LOG_PATH}\n"); fl.flush()

        schedule=[]
        for rep in range(1, REPEATS_PER_VARIANT+1):
            order=TOP_VARIANTS[:]
            random.shuffle(order)
            for v in order:
                schedule.append((v,rep))

        for v,rep in schedule:
            gi += 1
            row=None
            infra_retries=0
            ...
            line=(f"[{gi}/{total}] {row['variant_id']} rep={rep} v={row['victim_idx']} hot={row['hot_keys']} w={row['writers_per_key']} "
                  f"p={row['payload_mib']}MiB k={row['kill_delay_s']} hotf={row['hot_fail']} coldf={row['cold_fail']} "
                  f"strict={strict} any={anyh} rc={row['exit_code']} dur={row['duration_s']}s")
            print(line,flush=True); fl.write(line+'\n'); fl.flush()

    print(f"DONE iterations={total} strict_hits={strict} any_hits={anyh}")
